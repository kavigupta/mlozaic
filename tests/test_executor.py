...
class TestParsing(unittest.TestCase):
    ...
    def test_basic_circle(self):
        self.render_or_check("black-circle", "(circle 0 0 50 50)")

    def test_color(self):
        self.render_or_check("red-circle", "(color 255 0 0 (circle 0 0 50 50))")

    def test_square(self):
        self.render_or_check(
            "circle-and-square",
            "(combine (color 0 255 0 (circle 0 0 30 30)) (color 0 0 255 (square 0 0 20 20)))",
        )

    def test_line_of_circles(self):
        self.render_or_check(
            "line-of-circles",
            "(repeat $0 0 9 (circle (- (* $0 10) 40) 0 5 5))",
        )

    def test_rotated_square(self):
        self.render_or_check(
            "rotated-squares", "(repeat $0 0 3 (rotate (* $0 10) (square 0 0 30 30)))"
        )

    def test_parabola(self):
        self.render_or_check(
            "parabola",
            "(repeat $0 -30 30 (circle $0 (/ (* $0 $0) 30) 1 1))",
        )
    ...
    def test_star_field(self):
        self.render_or_check(
            ...
            """
            (combine
                (color 0 0 128 (square 0 0 100 100))
                (color 255 255 255
                    (repeat $0 -3 4
                        (repeat $1 -3 4
                            (ife (= (% (+ $0 $1) 2) 0)
                                (circle (* 12 $0) (* 12 $1) 5 5)
                                (square (* 12 $0) (* 12 $1) 5 5))))))
            """,
        )
